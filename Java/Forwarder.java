...
public class Forwarder implements Runnable {
	...
	public void run() {
	    ...
	    //       if the Substrate has an incoming packet
			...
			Pair<Packet, Integer> sp;
			Packet p;
			if (sub.incoming())
			{
				sp = sub.receive();
				if (sp.left.destAdr == myIp && sp.left.ttl > 0)
				{
					//application pkt
					if (sp.left.protocol == 1) toSnk.offer(sp.left);
					//router pkt
					else if (sp.left.protocol == 2) toRtr.offer(sp);
				}
				else if (sp.left.ttl > 0)
				{
					int nxtHop = lookup(sp.left.destAdr);
					sub.send(sp.left, nxtHop);
				}
			}
			else if (sp = fromRtr.peek() != null)
			{
				try {
					fromRtr.take();
				} catch(Exception e) {
					System.err.println("Forwarder:run: " +
						"can't read from router queue");
					System.exit(1);
				}
				if (sub.ready(sp.right)) sub.send(sp.left, sp.right);
			}
			else if (p = fromSrc.peek() != null)
			{
				try {
					fromSrc.take();
				} catch(Exception e) {
					System.err.println("Forwarder:run: " +
						"can't read from source queue");
					System.exit(1);
				}
				int nxtHop = lookup(p.destAdr);
				if (sub.ready(nxtHop)) sub.send(p, nxtHop);
			}
			else if (!sub.incoming() && fromRtr.peek() == null &&
					 fromSrc.peek() == null)
			{
	...
	public synchronized void addRoute(Prefix nuPrefix, int nuLnk) {
		...
		if (!contains) fwdTbl.add(new Pair<Prefix, Integer>(nuPrefix, nuLnk));
	...
	private synchronized int lookup(int ip) {
		for (Pair<Prefix, Integer> rte : fwdTbl)
		{
			int nxtHop = -1;
			int maxMatch = 0;
			if (rte.left.matches(ip) && rte.left.leng > maxMatch)
				nxtHop = rte.right; maxMatch = rte.left.leng;
		}
		return nxtHop;
	}
