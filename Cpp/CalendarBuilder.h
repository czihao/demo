...
public:
	virtual std::shared_ptr<DisplayableComponent> buildTODOlist() = 0;
	virtual std::shared_ptr<DisplayableComponent> buildTask(std::tm d, std::shared_ptr<DisplayableComponent> p, std::string, bool) = 0;
	virtual std::shared_ptr<DisplayableComponent> buildEvent(std::shared_ptr<Calendar> cal, std::string name, std::tm when, int recurrEvery, int recurrFor, std::string calendar) = 0;
	virtual std::shared_ptr<DisplayableComponent> buildEventMerge(std::string, std::shared_ptr<Calendar> cal, std::string name, std::tm when, int recurrEvery, int recurrFor) = 0;
